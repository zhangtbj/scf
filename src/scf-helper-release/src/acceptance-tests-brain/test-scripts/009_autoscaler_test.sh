...
function test_cleanup() {
    ...
    if test "${REMOVE_SERVICE}" = 1; then
        cf purge-service-offering -f "${SERVICE_OFFERING}"
        cf delete-service-broker -f app-autoscaler
    fi
...
if ! cf marketplace | grep "${SERVICE_OFFERING}" ; then
    REMOVE_SERVICE=1
    cf create-service-broker app-autoscaler username password "https://autoscalerservicebroker.${CF_DOMAIN}"
...
# test autoscaler
cf create-service "${SERVICE_OFFERING}" default "${SCALESERVICE}"
cf bind-service "${APP_NAME}" "${SCALESERVICE}" -c "$(cat "${POL}")"
cf restage "${APP_NAME}"
